...
task main()
{
	...
	while(true) {
		...
		setChain((vexRT[Btn6U] ^ vexRT[Btn6D]) //If exactly one of button 6 up or button 5 down is pressed
		...
		//Assign claw power
		setClaw((vexRT[Btn7L] ^ vexRT[Btn7R]) //If exactly one of button 6 up or button 6 down is pressed
			? (vexRT[Btn7L]) //If button 6 up is pressed
				? 127 //Assign power 127
				: -127 //If button 6 down is pressed (6 up is not, but one of the buttons is), assign power -127
			: 0); //If neither button is pressed, assign power 0
